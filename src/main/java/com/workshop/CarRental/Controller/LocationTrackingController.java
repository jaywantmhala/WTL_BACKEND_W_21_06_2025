// ===== BACKEND CODE UPDATE (Spring Boot) =====
...
public class LocationTrackingController {
    ...
    private final Map<Integer, LocationMessage> userLocations = new HashMap<>();
    private final Map<Integer, LocationMessage> driverLocations = new HashMap<>();

    @Autowired
    public LocationTrackingController(SimpMessagingTemplate messagingTemplate, BookingRepo bookingRepository,
            VendorDriverRepo vendorDriverRepository, CarRentalRepository userRepository) {
    ...
    public void updateLocation(LocationUpdateMessage message) {
        ...
        if ("DRIVER".equals(message.getRole())) {
            ...
            driverLocations.put(message.getBookingId(), createLocationMessage(message));
        } else {
            validateUserBookingAssociation(booking, message.getUserId());
            updateUserLocation(message);
            userLocations.put(message.getBookingId(), createLocationMessage(message));
        }

        // Send both locations back to both sides
        LocationMessage userLoc = userLocations.get(message.getBookingId());
        LocationMessage driverLoc = driverLocations.get(message.getBookingId());

        if (userLoc != null && driverLoc != null) {
            Map<String, Object> locationPayload = new HashMap<>();
            locationPayload.put("user", userLoc);
            locationPayload.put("driver", driverLoc);

            messagingTemplate.convertAndSend("/topic/location/" + message.getBookingId(), locationPayload);
        }
    }
